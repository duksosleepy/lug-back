...
def test_bc_kl_validation():
    ...
    try:
        ...
        if not result_df.empty:
            ...
            print(f"   ✅ Document Type: {first_entry.get('document_type', 'N/A')}")
            print(f"   ✅ Counterparty Code: {first_entry.get('counterparty_code', 'N/A')}")
            print(f"   ✅ Counterparty Name: {first_entry.get('counterparty_name', 'N/A')}")
            
            # Validate BC statement has KL counterparty
            if (first_entry.get('document_type') == 'BC' and 
                'KL' in str(first_entry.get('counterparty_code', ''))):
                print("   ✅ SUCCESS: BC transaction correctly assigned KL counterparty")
            else:
                print(f"   ❌ FAIL: BC transaction should have KL counterparty, got: {first_entry.get('counterparty_code')}")
                return False
        else:
        ...
        if not result_df_bn.empty:
            ...
            print(f"   ✅ Document Type: {first_entry_bn.get('document_type', 'N/A')}")
            print(f"   ✅ Counterparty Code: {first_entry_bn.get('counterparty_code', 'N/A')}")
            print(f"   ✅ Counterparty Name: {first_entry_bn.get('counterparty_name', 'N/A')}")
            
            # BN transactions should not be forced to KL
            if first_entry_bn.get('document_type') == 'BN':
                print("   ✅ SUCCESS: BN transaction processed (not forced to KL)")
            else:
                print(f"   ❌ FAIL: Expected BN transaction, got: {first_entry_bn.get('document_type')}")
                return False
        else:
        ...
        print("   ✅ Should see: 'BC statement validation passed' or 'Using default KL counterparty'")
...
def test_existing_bc_kl_filtering():
    ...
    mock_search_results = [
        {"code": "CUSTOMER001", "name": "Regular Customer", "address": "123 Street"},
        {"code": "KL-BARIA1", "name": "Khách Lẻ Bà Rịa 1", "address": "Bà Rịa - Vũng Tàu"},
    ...
    if document_type == "BC":
        filtered_counterparties = [
            cp for cp in mock_search_results 
            if cp.get("code") and "KL" in str(cp["code"])
        ]
        print(f"   ✅ Original counterparties: {len(mock_search_results)}")
        print(f"   ✅ Filtered for BC (containing 'KL'): {len(filtered_counterparties)}")
...
def test_pos_machine_kl_validation():
    ...
    print(f"   📋 Mock POS machine: {mock_pos_machines[0]['code']} with dept: {mock_pos_machines[0]['department_code']}")
    print(f"   🔄 Department replacements available: {list(extractor.department_code_replacements.keys())}")
    ...
    try:
        ...
        if result_bc:
            ...
            if 'KL' in str(result_bc.get('code', '')):
                ...
                print(f"      ⚠️  BC statement didn't get KL counterparty: {result_bc.get('code')}")
        else:
    ...
    try:
        ...
        if result_bn:
            ...
            print("      ✅ SUCCESS: BN statement processed without KL filtering")
        else:
    ...
    try:
        ...
        if result_none:
            print(f"      ✅ No doctype Result code: {result_none.get('code', 'N/A')}")
            print(f"      ✅ No doctype Result name: {result_none.get('name', 'N/A')}")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    if success:
        print("🎉 ALL TESTS PASSED! BC Statement KL validation is working correctly.")
        print("✅ BC statements will always have counterparty codes containing 'KL'")
