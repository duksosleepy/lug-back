...
class DaskExcelProcessor:
    def __init__(self, input_file: Union[str, io.BytesIO]):
        ...
        # Nếu input_file là đường dẫn thì sử dụng file hệ thống, ngược lại xử lý in-memory  # noqa: E501
    ...
    def read_input_file(self) -> dd.DataFrame:
        """Đọc và tiền xử lý dữ liệu từ file Excel (hỗ trợ file path hoặc file-like object)"""  # noqa: E501
        df = pd.read_excel(
            self.input_file,
            sheet_name="Sheet1",
    ...
    def save_output_file(self, df: dd.DataFrame) -> None:
        """Lưu kết quả ra file Excel (chỉ áp dụng khi xử lý theo file hệ thống)"""  # noqa: E501
        if self.output_file:
