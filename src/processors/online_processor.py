...
class DaskExcelProcessor:
    ...
    def _process_final(
        ...
        # Preserve existing Mã đơn hàng value from source file
    ...
    def _filter_kl_records(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        phone_mask = df["Số điện thoại"].astype(str).str.strip().isin(["0912345678"])
    ...
    def process_data(
        ...
        if not kl_records_df.empty:
            ...
            # Normalize date to dd/mm/yyyy string before JSON serialization
            kl_final_df["Ngày Ct"] = format_ngay_ct(kl_final_df["Ngày Ct"])
