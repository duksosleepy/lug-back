...
def find_header_row(df: pd.DataFrame) -> Tuple[int, Dict[str, int]]:
    ...
    header_patterns = [
        "so tham chieu", "so ct", "ma gd", "reference",  # Reference number
        "ngay hieu luc", "ngay hl", "ngay", "date",      # Date
        "ghi no", "debit", "tien ra", "chi",             # Debit
        "ghi co", "credit", "tien vao", "thu",           # Credit
        "so du", "balance",                              # Balance
        "mo ta", "dien giai", "noi dung", "description"  # Description
    ]
    ...
    for row_idx in range(min(20, len(df))):  # Look at first 20 rows
        ...
        for col_idx, cell_value in enumerate(row_values):
            if any(pattern in cell_value for pattern in header_patterns):
                ...
                if any(pattern in cell_value for pattern in ["so tham chieu", "so ct", "ma gd", "reference"]):
                    column_mapping["reference"] = col_idx
                elif any(pattern in cell_value for pattern in ["ngay hieu luc", "ngay hl", "ngay", "date"]):
                    column_mapping["date"] = col_idx
                elif any(pattern in cell_value for pattern in ["ghi no", "debit", "tien ra", "chi"]):
                    column_mapping["debit"] = col_idx
                elif any(pattern in cell_value for pattern in ["ghi co", "credit", "tien vao", "thu"]):
                    column_mapping["credit"] = col_idx
                elif any(pattern in cell_value for pattern in ["so du", "balance"]):
                    column_mapping["balance"] = col_idx
                elif any(pattern in cell_value for pattern in ["mo ta", "dien giai", "noi dung", "description"]):
                    column_mapping["description"] = col_idx
        
        # If we found at least 3 headers, consider this the header row
        if header_count >= 3:
            logger.info(f"Found header row at index {row_idx} with {header_count} headers")
...
def extract_transactions(df: pd.DataFrame, header_row: int, column_mapping: Dict[str, int]) -> pd.DataFrame:
    """
    ...
    for row_idx in range(start_row, len(df)):
        ...
        if any(term in row_text for term in ["tong cong", "total", "so du cuoi ky"]):
        ...
        # Only include rows that have a reference number
        if "reference" in record and pd.notna(record["reference"]) and record["reference"] != "":
            data_rows.append(record)
        else:
            logger.info(f"Skipping row {row_idx} - no reference number")
...
def process_transactions(transactions_df: pd.DataFrame) -> pd.DataFrame:
    """
    Process transactions into the required format for Saoke
    
    Args:
        transactions_df: DataFrame with the transactions
        
    Returns:
        DataFrame with processed data for Saoke format
    """
    # Document type counters for generating document numbers
    doc_counters = {"BC": 1, "BN": 1}
    ...
    for _, row in transactions_df.iterrows():
        try:
            ...
            if "credit" in row and pd.notna(row["credit"]) and row["credit"] > 0:
                is_credit = True
                amount = float(row["credit"])
            elif "debit" in row and pd.notna(row["debit"]) and row["debit"] > 0:
                is_credit = False
                amount = float(row["debit"])
            else:
                # Skip rows without transaction amounts
                continue
            
            # Generate document type and number
            doc_type = "BC" if is_credit else "BN"
            
            # Get month from date, default to current month
            month = datetime.now().month
            if "date" in row and pd.notna(row["date"]):
                date_obj = pd.to_datetime(row["date"])
                month = date_obj.month
            
            doc_number = f"{doc_type}{month:02d}/{doc_counters[doc_type]:03d}"
            ...
            record = {
                ...
                "reference": reference,
                "counterparty_code": "KL",  # Default
                "counterparty_name": "Khách Lẻ",  # Default
                "address": "",
                "description": description,
                "amount1": amount,
                "debit_account": "1121114" if is_credit else "6278",  # Default accounts
                "credit_account": "1311" if is_credit else "1121114",  # Default accounts
                "department": "",
                "cost_code": "",
            }
...
async def process_bank_statement(file: UploadFile):
    """
    Process BIDV bank statement file to Saoke format
    ...
    try:
        ...
        try:
            ...
            raw_df = pd.read_excel(input_buffer, header=None, engine="calamine")
            ...
            if "reference" not in column_mapping:
                logger.error("Reference column not found in header row")
                raise HTTPException(status_code=400, detail="Reference column not found in header row")
            
            # Extract transactions (only rows with reference numbers)
            transactions_df = extract_transactions(raw_df, header_row, column_mapping)
            
            logger.info(f"Extracted {len(transactions_df)} transactions with reference numbers")
            
            # Process transactions
            processed_df = process_transactions(transactions_df)
            
            logger.info(f"Processed {len(processed_df)} transactions")
            ...
            formatted_df["Ma_Ct"] = processed_df.get("document_type", "BC") if "document_type" in processed_df else "BC"
            formatted_df["Ngay_Ct"] = processed_df.get("date", "") if "date" in processed_df else ""
            formatted_df["So_Ct"] = processed_df.get("document_number", "") if "document_number" in processed_df else ""
            formatted_df["Ma_Tte"] = "VND"  # Always VND
            formatted_df["Ty_Gia"] = 1  # Always 1
            formatted_df["Ma_Dt"] = processed_df.get("counterparty_code", "") if "counterparty_code" in processed_df else ""
            formatted_df["Ong_Ba"] = processed_df.get("counterparty_name", "") if "counterparty_name" in processed_df else ""
            formatted_df["Dia_Chi"] = processed_df.get("address", "") if "address" in processed_df else ""
            formatted_df["Dien_Giai"] = processed_df.get("description", "") if "description" in processed_df else ""
            formatted_df["Tien"] = processed_df.get("amount1", 0) if "amount1" in processed_df else 0
            formatted_df["Tien_Nt"] = processed_df.get("amount1", 0) if "amount1" in processed_df else 0
            formatted_df["Tk_No"] = processed_df.get("debit_account", "") if "debit_account" in processed_df else ""
            formatted_df["Tk_Co"] = processed_df.get("credit_account", "") if "credit_account" in processed_df else ""
            ...
            formatted_df["Ngay_Ct0"] = processed_df.get("date", "") if "date" in processed_df else ""
            ...
            formatted_df["Ma_Bp"] = processed_df.get("department", "") if "department" in processed_df else ""
            formatted_df["Ma_Km"] = processed_df.get("cost_code", "") if "cost_code" in processed_df else ""
