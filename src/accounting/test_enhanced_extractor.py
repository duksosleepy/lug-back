    ...
    for i, desc in enumerate(test_descriptions, 1):
        ...
        if entity_info.get('counterparties'):
            ...
            for j, cp in enumerate(entity_info['counterparties'], 1):
                print(f"  {j}. {cp['name']} (Type: {cp['type']}, Confidence: {cp['confidence']:.2f})")
                
        if entity_info.get('accounts'):
            print(f"\nAccounts ({len(entity_info['accounts'])}):")
            for j, acc in enumerate(entity_info['accounts'], 1):
                print(f"  {j}. {acc['code']} (Type: {acc['type']}, Confidence: {acc['confidence']:.2f})")
        ...
        if entity_info.get('pos_machines'):
            ...
            for j, pos in enumerate(entity_info['pos_machines'], 1):
                print(f"  {j}. {pos['code']} (Type: {pos['type']}, Confidence: {pos['confidence']:.2f})")
                
        if entity_info.get('departments'):
            print(f"\nDepartments ({len(entity_info['departments'])}):")
            for j, dept in enumerate(entity_info['departments'], 1):
                print(f"  {j}. {dept['code']} (Type: {dept['type']}, Confidence: {dept['confidence']:.2f})")
            ...
            if entities:
                ...
                for i, entity in enumerate(entities, 1):
                    if entity_type == 'counterparties':
                        print(f"  {i}. {entity['name']} (Type: {entity['type']}, Confidence: {entity['confidence']:.2f})")
                    else:
                        print(f"  {i}. {entity['code']} (Type: {entity['type']}, Confidence: {entity['confidence']:.2f})")
        ...
        for entity_type, matches in matched_entities.items():
            if matches:
                ...
                for i, match in enumerate(matches, 1):
                    if entity_type == 'counterparties':
                        print(f"  {i}. {match.get('name', 'N/A')} (Code: {match.get('code', 'N/A')}, Score: {match.get('score', 0):.4f})")
                        print(f"     Extracted: {match.get('extracted_name', 'N/A')}")
                    else:
                        code_field = 'code' if 'code' in match else 'extracted_code'
                        print(f"  {i}. {match.get('name', 'N/A')} (Code: {match.get(code_field, 'N/A')}, Score: {match.get('score', 0):.4f})")
            else:
            ...
            for entity_type, matches in matched_entities.items():
                if matches:
                    ...
                    for j, match in enumerate(matches, 1):
                        if entity_type == 'counterparties':
                            print(f"  {j}. {match.get('name', 'N/A')} (Code: {match.get('code', 'N/A')}, Score: {match.get('score', 0):.4f})")
                            print(f"     Extracted: {match.get('extracted_name', 'N/A')}")
                        else:
                            code_field = 'code' if 'code' in match else 'extracted_code'
                            print(f"  {j}. {match.get('name', 'N/A')} (Code: {match.get(code_field, 'N/A')}, Score: {match.get('score', 0):.4f})")
